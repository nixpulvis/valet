        ...
        <section class={style}>
            ...
            <div>
                { if props.session.domain.is_none() {
                    ...
                    html! { for records.iter().map(|r| {
                        let copy = copy.clone();
                        let uuid = r.uuid().to_string();
                        html! {
                            <div class="record">
                                <span class="label">{ r.label().to_string() }</span>
...
/// A password input that Firefox won't autofill.
///
...
/// the value is stored as plain text in Yew state (same as before).
