...
fn show_toast(text: &str) {
    ...
    host.set_attribute(
        "style",
        &format!("position:fixed;top:12px;right:12px;z-index:{OVERLAY_Z_INDEX};pointer-events:none;"),
    )
    .unwrap();
    ...
    web_sys::window()
        .unwrap()
        .set_timeout_with_callback_and_timeout_and_arguments_0(fade.as_ref().unchecked_ref(), TOAST_VISIBLE_MS as i32)
        .unwrap();
    fade.forget();
}
