...
async fn handle_autofill_request(msg: JsValue) -> Result<JsValue, JsValue> {
    ...
    let records = match result {
        valet_native_host::RpcResult::Record(valet_native_host::RecordResult::List(r)) => r,
...
async fn handle_autofill_fill(msg: JsValue) -> Result<JsValue, JsValue> {
    ...
    let record = match result {
        valet_native_host::RpcResult::Record(valet_native_host::RecordResult::Get(r)) => r,
        other => return autofill_error(&format!("unexpected response: {other:?}")),
    };
...
async fn handle_autofill_generate(msg: JsValue) -> Result<JsValue, JsValue> {
    ...
    .await?;
    let record = match result {
        valet_native_host::RpcResult::Record(valet_native_host::RecordResult::Generated(r)) => r,
        other => return autofill_error(&format!("unexpected response: {other:?}")),
    };
...
fn autofill_credential_response(record: &valet::Record) -> Result<JsValue, JsValue> {
    let credential_username = match record.label() {
        valet::record::Label::Domain { id, .. } => Some(id.clone()),
        valet::record::Label::Simple(s) => Some(s.clone()),
    };
...
async fn first_unlocked_user() -> Result<String, JsValue> {
    ...
    let result =
        valet_native_host::decode_result(&b64).map_err(|e| JsValue::from_str(&e.to_string()))?;
    match result {
        valet_native_host::RpcResult::Unlocked(users) => users
            .into_iter()
            .next()
...
/// A single RPC frame sent to the native host over the [`Port`](super::externs::Port).
#[derive(Serialize)]
...
pub(crate) async fn call_native(method: &str, params: JsValue) -> Result<String, String> {
    ...
    let serializer = serde_wasm_bindgen::Serializer::json_compatible();
    let frame_js = serde::Serialize::serialize(&frame, &serializer).map_err(|e| e.to_string())?;
...
/// Call the native host and decode the base64 response into an [`RpcResult`].
///
...
async fn call_native_rpc(
    method: &str,
    params: serde_json::Value,
) -> Result<valet_native_host::RpcResult, JsValue> {
    let params_js =
