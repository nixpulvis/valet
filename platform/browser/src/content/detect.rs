...
fn scan() {
    ...
    for i in 0..pw_fields.length() {
        ...
        if html_el.offset_width() < MIN_VISIBLE_WIDTH
            || html_el.offset_height() < MIN_VISIBLE_HEIGHT
        {
...
fn find_username_field(pw: &HtmlInputElement) -> Option<HtmlInputElement> {
    ...
    if best.is_none() {
        for i in 0..candidates.length() {
            ...
            let html: &web_sys::HtmlElement = input.as_ref();
            if html.offset_width() >= MIN_VISIBLE_WIDTH
                && html.offset_height() >= MIN_VISIBLE_HEIGHT
            {
                return Some(input);
            }
        }
