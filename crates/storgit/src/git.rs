...
impl<'a> BareRepo<'a> {
    ...
    /// Like [`Self::merge_in_progress`] but errors instead of
    /// returning a bool. Both layouts call this at the top of their
    /// merge kernels.
    pub(crate) fn ensure_no_merge_in_progress(&self) -> Result<(), Error> {
    ...
    /// Path to the `MERGE_TREE` marker. Stores the auto-merged tree
    /// oid the kernel produced before pausing on conflicts, so the
    /// resolution path can apply pick overrides without re-running
    /// the three-way tree merge.
    pub(crate) fn merge_tree_path(&self) -> PathBuf {
        self.path.join("MERGE_TREE")
    }

    pub(crate) fn read_merge_tree(&self) -> Result<Option<gix::ObjectId>, Error> {
        read_ref_file(&self.merge_tree_path())
    }

    pub(crate) fn write_merge_tree(&self, oid: gix::ObjectId) -> Result<(), Error> {
        std::fs::write(self.merge_tree_path(), format!("{oid}\n"))?;
        Ok(())
    }

    pub(crate) fn clear_merge_tree(&self) -> Result<(), Error> {
        match std::fs::remove_file(self.merge_tree_path()) {
            Ok(()) => Ok(()),
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => Ok(()),
            Err(e) => Err(e.into()),
        }
